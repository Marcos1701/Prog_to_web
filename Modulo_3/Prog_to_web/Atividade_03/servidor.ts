...
class Jogo {
    ...
    get jogador_atual(): Jogador {
        if(this._jogadores.length == 0){
            throw new Error("Não há jogadores cadastrados")
        }
...
//                 } else {
//                     socket.write("Valor inválido, digite novamente: ")
...
//     let opcao: string = tratarEntradaUsuario(socket, ["0", "1"], opcoes);

//     if (opcao === "1") {
//         Game(socket)
//     } else {
//         socket.write("Obrigado por jogar, até mais!\n")
//         socket.end()
//     }
...
// socket.on('data', (data: Buffer) => {
//     let opcao: string = tratarEntradaUsuario(socket, ['1', '0'], opcoes);
